...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    da_prev = np.dot(parameters["Wf"].T[:n_a, :], dft) + np.dot(parameters["Wi"].T[:n_a, :], dit) + np.dot(
        parameters["Wc"].T[:n_a, :], dcct) + np.dot(parameters["Wo"].T[:n_a, :], dot)
    dc_prev = (da_next * ot * (1 - np.tanh(c_next) ** 2) + dc_next) * ft
    dxt = np.dot(parameters["Wf"].T[n_a:, :], dft) + np.dot(parameters["Wi"].T[n_a:, :], dit) + np.dot(
        parameters["Wc"].T[n_a:, :], dcct) + np.dot(parameters["Wo"].T[n_a:, :], dot)
